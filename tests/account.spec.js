...
const {validAccountHolderName, invalidAccountHolderName,validBalance,invalidBalanceString,invalidBalanceZero,invalidBalanceNegative,invalidBalanceBoolean,invalidValueOnCreationError,setNameInvalidUsernameError} = testData
...
// Jest will then call the function and correctly catch and verify the error.
    ...
    expect(()=>{new Account(invalidAccountHolderName,validBalance)}).toThrow(invalidValueOnCreationError);
})
...
test('error:cannot add invalid balance negative integer',()=>{
    let testAccount = new Account(validAccountHolderName,validBalance)
    expect(()=>{testAccount.addFunds(-1)}).toThrow("Please enter a valid amount");
})

test('error:cannot add invalid balance string input',()=>{
    let testAccount = new Account(validAccountHolderName,validBalance)
    expect(()=>{testAccount.addFunds("one")}).toThrow("Please enter a valid amount");
})

test('error:cannot add invalid balance input:0',()=>{
    let testAccount = new Account(validAccountHolderName,validBalance)
