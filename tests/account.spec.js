...
test("should throw an error if initialDeposit is 0",()=>{
    ...
    expect(()=>{new Account(testData.validAccountHolderName,testData.invalidBalanceBoolean)}).toThrow(testData.invalidValueOnCreationError);
})

    //Getter tests;

    test("should return correct balance",()=>{
    let testAccount = new Account(testData.validAccountHolderName,testData.validBalance)
    expect(testAccount.balance).toBe(100)
})
     test('should return correct account number', () => {
        const account = new Account(testData.validAccountHolderName, testData.validBalance);
    ...
    test('should throw an error if new name is not a string', () => {
        const account = new Account(testData.validAccountHolderName, testData.validBalance);
       expect(()=>account.accountHolder=123).toThrow(testData.setNameInvalidUsernameError);
    });
